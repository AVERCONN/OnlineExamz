...
public class DelstServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String url = "jdbc:mysql://localhost:3307/examinationsystem?user=root&password=123";
		 ...
		 try{ 
			...
			if(rs.next()){	//如果找到这条记录，则得到这些字段的值放在stInfo的成员变量中
		...
		//把得到的值保存在session中
		 ...
		 request.getRequestDispatcher("deletest1.jsp").forward(request,response);//跳转到删除考生信息页面
	}
