...
public class StChangeServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

    public StChangeServlet() {
    	super();
    }
  ...
  String sql="update student set sname= '"+sname+"',xueli= '"+xueli+"',sex= '"+sex+"',card= '"+card+"',address= '"+address+"',university= '"+university+"',email= '"+email+"',password= '"+password+"',special= '"+special+"',telephone= '"+telephone+"' where username= '"+username+"'";//修改数据
			
				int a = stmt.executeUpdate(sql);
				if(a>0){
					 out.print("<head><title>考生管理员--修改考生信息</title></head>");
			    out.print("考生信息修改成功！");	
				}
else{
					out.print("考生信息修改失败！");	
