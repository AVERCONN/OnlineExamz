...
/**
 * Servlet implementation class EditPaperServletIII
 */
public class EditPaperServletIII extends HttpServlet {               //编辑试卷功能中提交更改界面
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EditPaperServletIII() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		paper.setpChoShu(pChoShu);           //设置各数值
		...
		for(int i=0;i<_pChoShu;i++) {
			...
			try {
				...
				pNo=paperDao.getPaperNo(pName);
				if(paperDao.updateChoiceQu(paper, pNo, i+1)==1) {                //向数据库写入选择题
		...
		for(int i=0;i<_pFillShu;i++) {
			...
			try {
				paperDao.getConn(server, db, user, pwd);
				pNo=paperDao.getPaperNo(pName);
				if(paperDao.updateFillQu(paper, pNo, j)==1) {                //向数据库写入填空题
		...
		out.print("<input type='submit' value='浏览试卷'  />");
